...
class ModuleCrud extends Command
{
     ...
     * @var string
     */
    protected $description = 'Generate a CRUD module for quicksite';
    ...
    public function handle()
    {
        ...
        $moduleDirectory = base_path('quicksite/modules/'.ucfirst(str_plural($table)));
        ...
        file_put_contents($moduleDirectory.'/Views/menu.blade.php', "<li class=\"@if (Request::is('quicksite/".str_plural($table)."') || Request::is('quicksite/".str_plural($table)."/*')) active @endif\"><a href=\"<?= URL::to('quicksite/".strtolower(str_plural($table))."'); ?>\"><span class=\"fa fa-file\"></span> ".ucfirst(str_plural($table)).'</a></li>');
        ...
        $config = [
            ...
            'routes_prefix'              => "<?php \n\nRoute::group(['namespace' => 'Quicksite\Modules\\".ucfirst(str_plural($table))."\Controllers', 'prefix' => 'quicksite', 'middleware' => ['web', 'auth', 'quicksite']], function () { \n\n",
            'routes_suffix'              => "\n\n});",
            '_app_namespace_'            => app()->getInstance()->getNamespace(),
            '_namespace_services_'       => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Services',
            '_namespace_facade_'         => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Facades',
            '_namespace_model_'          => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Models',
            '_namespace_model_'          => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Models',
            '_namespace_controller_'     => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Controllers',
            '_namespace_request_'        => 'Quicksite\Modules\\'.ucfirst(str_plural($table)).'\Requests',
        ...
        Artisan::call('make:migration', [
            ...
            '--path'   => 'quicksite/modules/'.ucfirst(str_plural($table)).'/Publishes/database/migrations',
            '--table'  => str_plural(strtolower($table)),
            '--create' => true,
        ]);

        if ($this->option('schema')) {
            $migrationFiles = $filesystem->allFiles(base_path('quicksite/modules/'.ucfirst(str_plural($table)).'/Publishes/database/migrations'));
