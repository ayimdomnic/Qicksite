...
@section('content')
    ...
    <div class="row">
        <a class="btn btn-primary pull-right" href="{!! route('quicksite.blog.create') !!}">Add New</a>
        <div class="raw-m-hide raw-m-hide pull-right">
            {!! Form::open(['url' => 'quicksite/blog/search']) !!}
        ...
        @else
            <table class="table table-striped">
                ...
                @foreach($blogs as $blog)
                    <tr>
                        <td><a href="{!! route('quicksite.blog.edit', [$blog->id]) !!}">{!! $blog->title !!}</a></td>
                        ...
                        <td class="text-right">
                            <form method="post" action="{!! url('quicksite/blog/'.$blog->id) !!}">
                                {!! csrf_field() !!}
                                {!! method_field('DELETE') !!}
                                <button class="delete-btn btn btn-xs btn-danger pull-right" type="submit"><i class="fa fa-trash"></i> Delete</button>
                            </form>
                            <a class="btn btn-xs btn-default pull-right raw-margin-right-8" href="{!! route('quicksite.blog.edit', [$blog->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
