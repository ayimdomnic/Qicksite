...
@section('content')
    ...
    <div class="row">
        <a class="btn btn-primary pull-right" href="{!! route('quicksite.widgets.create') !!}">Add New</a>
        <div class="raw-m-hide pull-right">
            {!! Form::open(['url' => 'quicksite/widgets/search']) !!}
        ...
        @else
            <table class="table table-striped">
                ...
                @foreach($widgets as $widget)
                    <tr>
                        <td><a href="{!! route('quicksite.widgets.edit', [$widget->id]) !!}">{!! $widget->name !!}</a></td>
                        <td class="raw-m-hide">{!! $widget->slug !!}</td>
                        <td>
                            <form method="post" action="{!! url('quicksite/widgets/'.$widget->id) !!}">
                                {!! csrf_field() !!}
                                {!! method_field('DELETE') !!}
                                <button class="delete-btn btn btn-xs btn-danger pull-right" type="submit"><i class="fa fa-trash"></i> Delete</button>
                            </form>
                            <a class="btn btn-xs btn-default pull-right raw-margin-right-8" href="{!! route('quicksite.widgets.edit', [$widget->id]) !!}"><i class="fa fa-pencil"></i> Edit</a>
                        </td>
                    </tr>
                @endforeach
                </tbody>
